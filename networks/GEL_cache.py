...
class TransformerModel(nn.Module):
    def __init__(
        ...
        # 证据学习修改self.decoders和self.g_decoder即可
        
        self.decoders = MLP_evidential(int(self.hidden_dim / 2),
                            self.hidden_dim,
                            self.input_dim,
                            self.n_layers,
                            self.dropout_rate)
        # 移除邻接矩阵重构解码器
        self.g_decoder = InnerProductDecoder()
        ...
        self.mulinf = MLP(int(self.hidden_dim *2),
                            self.hidden_dim,
                            1,
                            2,
                            self.dropout_rate)
    def forward(self, batched_data):
        ...
        #neighbor_tensor = torch.squeeze(self.Maxread(neighbor_tensor))
        neighbor_tensor = torch.sum(neighbor_tensor, dim=1, keepdim=True)

        output = (node_tensor + neighbor_tensor).squeeze()

#       rec_x的维度是(batch_size, input_dim)
        # pred包含了mu, v, alpha, beta
        pred = self.decoders(self.out_proj(output))
        mu, v, alpha_evidential, beta = (d.squeeze() for d in pred)
        rec_x = mu
        # 移除邻接矩阵重构部分
        rec_g = self.g_decoder(output,activation=torch.sigmoid)
        MINE = self.mulinf(torch.cat([node_tensor , neighbor_tensor],dim=2).squeeze())
        MINE_prime = self.mulinf(torch.cat([node_tensor, neighbor_tensor[torch.randperm(neighbor_tensor.size()[0])]], dim=2).squeeze())
        return node_tensor,neighbor_tensor,rec_x,rec_g,MINE,MINE_prime, mu, v, alpha_evidential, beta, pred



class TransformerModel_wodec(nn.Module):
    def __init__(
            self,
            hops,
            n_class,
    ...
    def forward(self, batched_data):
        ...
        layer_atten = self.attn_layer(torch.cat((target, neighbor_tensor), dim=2))
        #layer_atten = self.attn_layer(neighbor_tensor)
        layer_atten = F.softmax(layer_atten, dim=1)
...
class EncoderLayer2(nn.Module):
    def __init__(self, hidden_size, ffn_size, dropout_rate, attention_dropout_rate, num_heads):
        ...
        self.self_attention_norm = nn.LayerNorm(hidden_size)
        self.encoders = nn.Linear(hidden_size,hidden_size,bias=True)
...
class MLP_evidential(nn.Module):
    def __init__(self,
                 in_feats,
                 n_hidden,
                 n_hidden2,
                 n_layers,
                  dropout):
        super(MLP_evidential, self).__init__()
        self.layers = nn.ModuleList()
        # input layer
        self.layers.append(
            nn.Sequential(nn.Linear(in_feats, n_hidden),
        ...
        # output layer
        self.layers.append(nn.Linear(n_hidden, n_hidden2))
        self.layers.append(NormalInvGamma(n_hidden2, n_hidden2))
        self.dropout = nn.Dropout(p=dropout)
